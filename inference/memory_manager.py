...
class MemoryManager:
    ...
    def match_memory(self, query_key, selection):
        ...
        query_key = flatten(query_key, start_dim=2)
        selection = flatten(selection, start_dim=2) if selection is not None else None
        ...
        if self.enable_long_term and self.long_mem.engaged():
            ...
            memory_key = np.concatenate([self.long_mem.key, self.work_mem.key], -1)
            shrinkage = np.concatenate([self.long_mem.shrinkage, self.work_mem.shrinkage], -1) 
            ...
            affinity, usage = do_softmax(
                    np.concatenate([long_mem_similarity[:, -self.long_mem.get_v_size(0):], work_mem_similarity], 1), 
            ...
            for gi in range(1, num_groups):
                if gi < self.long_mem.num_groups:
                    ...
                    affinity_one_group = do_softmax(
                        np.concatenate([long_mem_similarity[:, -self.long_mem.get_v_size(gi):], 
            ...
            for gi, gv in enumerate(self.work_mem.value):
                ...
                if gi < self.long_mem.num_groups:
                    all_memory_value.append(np.concatenate([self.long_mem.value[gi], self.work_mem.value[gi]], -1))
                else:
        ...
        all_readout_mem = np.concatenate([
            self._readout(affinity[gi], gv)
            for gi, gv in enumerate(all_memory_value)
        ], 0)

        return all_readout_mem.reshape(all_readout_mem.shape[0], self.CV, h, w)
    ...
    def add_memory(self, key, shrinkage, value, objects, selection=None):
        ...
        key = flatten(key, start_dim=2)
        shrinkage = flatten(shrinkage, start_dim=2) 
        value = flatten(value[0], start_dim=2)
        ...
        if selection is not None:
            ...
            selection = flatten(selection, start_dim=2)
    ...
    def create_hidden_state(self, n, sample_key):
        ...
        if self.hidden is None:
            self.hidden = np.zeros((1, n, self.hidden_dim, h, w))
        elif self.hidden.shape[1] != n:
            self.hidden = np.concatenate([
                self.hidden, 
                np.zeros((1, n-self.hidden.shape[1], self.hidden_dim, h, w))
            ], 1)
    ...
    def consolidation(self, candidate_key, candidate_shrinkage, candidate_selection, usage, candidate_value):
        ...
        _, max_usage_indices = topk(usage, k=self.num_prototypes, dim=-1, sorted=True)
