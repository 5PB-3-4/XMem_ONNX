...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        image = unsqueeze(image, 0) # add the batch dimension
        ...
        if need_segment:
            memory_readout = unsqueeze(self.memory.match_memory(key, selection), 0)
        ...
        if is_mem_frame:
            value, hidden = self.network.encode_value(image, f16, self.memory.get_hidden(), 
                                    unsqueeze(pred_prob_with_bg[1:], 0), is_deep_update=is_deep_update)
