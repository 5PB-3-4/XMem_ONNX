...
class KeyValueMemoryStore:
    ...
    def add(self, key, value, shrinkage, selection, objects: List[int]):
        new_count = np.zeros((key.shape[0], 1, key.shape[2]), dtype=np.float32)
        new_life = np.zeros((key.shape[0], 1, key.shape[2]), dtype=np.float32) + 1e-7
        ...
        if self.k is None:
            ...
            self.k = np.concatenate([self.k, key], -1)
            if shrinkage is not None:
                self.s = np.concatenate([self.s, shrinkage], -1)
            if selection is not None:
                self.e = np.concatenate([self.e, selection], -1)
            if self.count_usage:
                self.use_count = np.concatenate([self.use_count, new_count], -1)
                self.life_count = np.concatenate([self.life_count, new_life], -1)

        # add the value
        if objects is not None:
            # When objects is given, v is a ndarray; used in working memory
            ...
            for gi, group in enumerate(self.obj_groups):
                ...
                self.v[gi] = np.concatenate([self.v[gi], value[group]], -1)
            ...
            for gi, gv in enumerate(value):
                ...
                if gi < self.num_groups:
                    self.v[gi] = np.concatenate([self.v[gi], gv], -1)
                else:
                    self.v.append(gv)

    def update_usage(self, usage:np.ndarray):
        # increase all life count by 1
        # increase use of indexed elements
        if not self.count_usage:
            return
        
        self.use_count += usage.reshape(self.use_count.shape)
        self.life_count += 1
    ...
    def sieve_by_range(self, start: int, end: int, min_size: int):
        ...
        if end == 0:
            ...
            self.k = np.concatenate([self.k[:,:,:start], self.k[:,:,end:]], -1)
            if self.count_usage:
                self.use_count = np.concatenate([self.use_count[:,:,:start], self.use_count[:,:,end:]], -1)
                self.life_count = np.concatenate([self.life_count[:,:,:start], self.life_count[:,:,end:]], -1)
            if self.s is not None:
                self.s = np.concatenate([self.s[:,:,:start], self.s[:,:,end:]], -1)
            if self.e is not None:
                self.e = np.concatenate([self.e[:,:,:start], self.e[:,:,end:]], -1)
            
            for gi in range(self.num_groups):
                if self.v[gi].shape[-1] >= min_size:
                    self.v[gi] = np.concatenate([self.v[gi][:,:,:start], self.v[gi][:,:,end:]], -1)

    def remove_obsolete_features(self, max_size: int):
        # normalize with life duration
        usage = self.get_usage().flatten()

        values, _ = topk(usage, k=(self.size-max_size), largest=False, sorted=True)
